	...
	*/
    ...
    private class CameraTilterThread extends Thread {
        ...
        public void run() {
            ...
            state.set(oculusPrime.State.values.cameratilt, camTargetPosition-1); // force horiz position move on startup
            boolean released = false;

            while (true) {

                int currentpos = state.getInteger(oculusPrime.State.values.cameratilt);
                ...
                if (currentpos == camTargetPosition) {
                    if (!released) {
                        ...
                        if (currentpos == camTargetPosition) {
                            application.messageplayer(null, "cameratilt", state.get(oculusPrime.State.values.cameratilt));
                            released = true;
...
//                while (commandList.size() > 0 && System.currentTimeMillis() < stopwaiting) Util.delay(1);
                ...
                state.set(oculusPrime.State.values.cameratilt, newposition);
