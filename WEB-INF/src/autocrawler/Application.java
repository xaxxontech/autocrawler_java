...
public class Application implements ServletContextListener {
 ...
 // moved to nav ---       state.set(State.values.navsystemstatus, Ros.navsystemstate.stopped.toString());
	...
	public void driverSignIn(String username, long clientID) {
        ...
        messageplayer(username + " connected to AUTOCRAWLER", "multiple", str);
        initialstatuscalled = false;
...
//	    Util.debug("playerCallServer "+fn.toString()+" "+str, this);
		...
		case move:  move(str, passengerOverride); break;
...
//		case muterovmiconmovetoggle: muteROVMicOnMoveToggle(); break;
		...
		case spotlightsetbrightness: break; // deprecated, maintained for mobile client compatibility
		case spotlight: 
...
//            } catch (Exception e) {}
	...
	private void getDrivingSettings() {
		if (loginRecords.isAdmin()) {
			String str = comport.speedslow + " " + comport.speedmed + " "
                    ...
                    + settings.readSetting(GUISettings.reversesteeringcomp.name()) + " "
                    + comport.CAM_MAX + " " + comport.CAM_HORIZ + " " + comport.CAM_MIN;
	...
	private void drivingSettingsUpdate(String str) {
		if (loginRecords.isAdmin()) {
            ...
            comport.setReverseSteeringComp(comps[8]);
...
//		if (!state.getBoolean(State.values.motionenabled)) {
		...
		switch (dir) {
            case forward: comport.goForward(); break;
