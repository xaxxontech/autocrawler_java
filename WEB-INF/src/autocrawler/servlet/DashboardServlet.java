...
public class DashboardServlet extends HttpServlet implements Observer {
	...
	public static String getCSS(){
		...
		try {	
			String line = null;
			FileInputStream filein = new FileInputStream(System.getenv("RED5_HOME") + "/webapps/autocrawler/dashboard.css");
			...
			new Thread(new Runnable() { public void run() {
				...
				if(action.equalsIgnoreCase("batterylog")){
					...
					*/
					...
					// for( int i = 0 ; i < t.size() ; i++ ) Util.debug((String)t.get(i));
					// Util.delay(300);
					// try { response.sendRedirect("/autocrawler/dashboard"); } catch (IOException e) { e.printStackTrace(); }
...
//					if(Navigation.turnLightOnIfDark()) Util.log("light was turned on because was dark", this);
				...
				if(action.equalsIgnoreCase("save")) {
					if( ! new Downloader().FileDownload("http://" + state.get(values.localaddress)
							+ ":" + httpport + "/autocrawler/dashboard?action=snapshot", "snapshot_"+ System.currentTimeMillis() +".txt", "log"))
			...
			response.sendRedirect("/autocrawler/dashboard");
		}
		...
		if(view != null){
			...
			if(view.equalsIgnoreCase("users")){
				...
				out.println("<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=windows-1252\"><title>Autocrawler log file</title> \n");
			...
			if(view.equalsIgnoreCase("routes")){
				out.println("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\n");
				out.println("<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=windows-1252\"><title>Autocrawler Route Statistics</title> \n");
				out.println("<style type=\"text/css\">");
...
//					txt = Util.trimLength(txt, MAX_LINE_LENGTH);
					txt = txt.replaceFirst("autocrawler.", "");
					txt = txt.replaceFirst(",", "");

					// TODO: CSS 
					if(txt.startsWith("AUTOCRAWLER:"))     out.println("<tr><td>autocrawler<td>"+ Util.trimLength(txt.substring(7), MAX_LINE_LENGTH) +"\n");
		...
		// default view 
		out.println("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\n");
		out.println("<html>\n<head><meta http-equiv=\"refresh\" content=\"20\">\n<title>Autocrawler</title>\n<style type=\"text/css\">"+ css +"</style></head>\n<body> \n");
		out.println(toDashboard(request.getServerName()+":"+request.getServerPort() + "/autocrawler/dashboard") + "\n");
		out.println("\n</body></html>\n");
		out.close();
	}
...
//			text.append(PowerLogger.tail(45) + "\n");
			...
			new SendMail("autocrawler snapshot", text.toString());
...
//		out.println(PowerLogger.tail(99).replaceAll("<br>",  "\n") + "\n");
	...
	public String toDashboard(final String url){
		...
		str.append("\n<td class='menu'>&nbsp;&nbsp;ssid<td>&nbsp;&nbsp;<a href=\"/autocrawler/network\">" + state.get(values.ssid));
		...
		else str.append("<td class='menu'>&nbsp;&nbsp;wan<td>&nbsp;&nbsp;<a href=\"http://"+ ext + ":" + httpport + "/autocrawler/\">" + ext + "</a>");
		str.append( "<td class='menu'>&nbsp;&nbsp;hdd<td>&nbsp;&nbsp;" + hdd + "% used</a></tr> \n");

		// power | lan | prime 
		str.append("\n<tr><td class='menu'>&nbsp;&nbsp;power<td>&nbsp;&nbsp;" + power );
		str.append("<td class='menu'>&nbsp;&nbsp;lan<td>&nbsp;&nbsp;<a href=\"http://"+state.get(values.localaddress) + ":" + httpport + "/autocrawler/\">" + state.get(values.localaddress) + "</a>");
...
//		if(state.getBoolean(values.routeoverdue))  flags += " * overdue* "; 		
	...
	public static String tailFormated(int lines){
		...
		for(int i = 0; i < history.size() ; i++) {
			...
			line = line.replaceFirst("AUTOCRAWLER:", "");
			line = line.replaceFirst("[INFO]", "");
			line = line.replaceFirst("autocrawler.", "");
			line = line.replaceFirst("Outocrawler", "");
