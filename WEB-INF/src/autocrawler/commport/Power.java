...
//	protected String version = null;
	...
	public Power(Application app) {
		...
		//ERROR CODES, WARNING NO CHARGE (20-39):						 // changed, was safe charge
		temp.put("ERROR_MAX_PWM_BUT_LOW_CURRENT", 					21); // (MOVED, was 42)
...
//				if (now - lastReset > RESET_DELAY && isconnected) PowerLogger.append(FIRMWARE_ID+" past reset delay", this);
				...
				if (now - lastRead > ERROR_TIME_OUT && !isconnected) { // comm with pcb lost!
					if (state.exists(autocrawler.State.values.powererror.toString())){
						String err = state.get(autocrawler.State.values.powererror);
						if (!err.matches(".*"+COMM_LOST+"$")) { // only set once
							...
							if (!isconnected) { // still not connected after waiting for reset
								state.set(autocrawler.State.values.powererror, err + "," + COMM_LOST);
							}
						...
						if (!isconnected) // still not connected after waiting for reset
							state.set(autocrawler.State.values.powererror, COMM_LOST);
	...
	private void timeoutlogmessages() {
		state.set(autocrawler.State.values.batterylife, "TIMEOUT");
